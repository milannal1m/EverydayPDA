...
class AnswerProcessor:
    ...
    async def __fetch_from_database(self, key: str, user_id: str) -> Optional[str]:
        """
        Fetch missing values from the PostgreSQL database based on the username.
        """
        query_map = {
    ...
    def __get_default_value(self, key: str) -> str:
        """
        Return default values for information not provided by the user.
        """
        default_values = {
    ...
    async def __fill_missing_values(self, data: Dict[str, str], user_id: str) -> Dict[str, str]:
        """
    ...
    async def __get_use_cases_and_info(self, message: str, user_id: str):
        """
        Extract use cases and required information from the message.
        """
    ...
    def __call_apis(self, use_cases, information_got):
        """
        Call APIs for the given use cases and information.
        """
        results = {}
    ...
    async def __get_api_data_without_gpt(self, use_cases, user_id: str):
        """
        Fetch API data without using GPT for the given use cases and user ID.
        """
    ...
    async def __get_user_morning(self, user_id: str):
        """
        Generate a morning summary for the user.
        """
        use_case_processor = UseCaseProcessor()
    ...
    def __get_stocks_with_significant_change(self, stocks):
        ...
        for symbol, data in stocks.items():
            change_1_hour = data.get("change1hour", 0)
            if change_1_hour:
                if abs(float(change_1_hour)) > 1:
                    significant_stocks.append(data)
        return significant_stocks

    def __is_within_last_hour(self, timestamp_str: str) -> bool:
        """
        Check if the given timestamp is within the last hour.
        """
    ...
    async def __get_user_proactivity(self, user_id: str):
        """
        Generate proactive suggestions for the user based on stocks and news.
        """
        use_case_processor = UseCaseProcessor()
        ...
        if significant_stocks or new_news:
            message = message = "Stell dir vor du bist proaktiv und erzählst mir etwas Neues über meine Aktien oder News. Erwähne bei den Aktien, wie sie sich in der letzten Stunde verändert haben. Beginne mit Hey, hast du schon gehört?"
            response = use_case_processor.response(message, api_data)
    ...
    async def get_morning(self) -> Dict[str, List[Dict[str, str]]]:
        ...
        for record in user_ids:
            ...
            try:
                ...
                results.append({
                    "user_id": str(user_id),
                    "response": morning_result.get("response", "Error retrieving data")
                })
    ...
    async def get_proactivity(self) -> Dict[str, List[Dict[str, str]]]:
        ...
        for record in user_ids:
            ...
            try:
                ...
                results.append({
                    "user_id": str(user_id),
                    "response": proactivity_result.get("response", "Error retrieving data")
                })
            except Exception as e:
                results.append({
                    "user_id": str(user_id),
